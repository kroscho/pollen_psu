...
const TestEdit = ({isVisible, setIsVisible}) => {
    ...
    const [fetchTest, isDataLoading, testError] = useFetching(async () => {
        let response = await TestingApi.getTestWithAnswers(curTest.nameTest);
    ...
    const handleOk = () => {
        const isChanges = deepEqual(form.getFieldsValue(), curTest)
        setIsVisible(false);
    };

    const handleCancel = () => {
        form.setFieldsValue({ nameTest: curTest.nameTest, tasks: curTest.tasks, answers: curTest.tasks})
        setIsVisible(false);
    };
